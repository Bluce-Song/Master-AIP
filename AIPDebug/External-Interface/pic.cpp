...
void pic::Pubs_from_main(QStringList io_List, int value, int flag) {
    ...
    case 2:
        ...
        if (Test_Model_Vacuum) { // 放气
            if (L_Stop) {
                qDebug() << "L_Stop L_Cylinder_Up()";
                L_Cylinder_Up();
            } else {
                qDebug() << "L_Stop L_Exhaust_Air()";
                L_Exhaust_Air();
            }
        } else {
            if (Test_MAG && MAG_Drive) {
...
void pic::Get_H_In(int in_h) {
    ...
    case IN_L_Light_Curtain:
        if (IN_L_Light_Curtain & IO_in_H) {
            ...
            if ((L_Test_Product) && (Test_Start_Auto)) {
                ...
                qDebug() << "L_Test_Product" << L_Test_Product <<\
    ...
    case IN_R_Start:
        ...
        if (IN_R_Light_Curtain & IO_in_H) {
            qDebug() << "R-R_Light_Curtain  -----Return";
            break;
        } else {
            //
        }
        if ((R_Cylinder_Uping) || (!R_Horizontal_Reset)) {
            qDebug() << "R-return----R_Cylinder_Uping  ||  R_Horizontal_Reset" \
        ...
        if (R_Test_Product) {
            qDebug() << "R_Test_Product -----Return";
        } else {
            Signal_pic_to_Main(QString(""), 18, 10);
            break;
        }
        if (IN_R_Start & IO_in_H) { qDebug() << "R-Start";
            if (System_ing) {
    ...
    case IN_L_Start:
        ...
        if (IN_L_Light_Curtain & IO_in_H) {
            qDebug() << "L-L_Light_Curtain -----Return";
        ...
        if ((L_Cylinder_Uping) || (!L_Horizontal_Reset)) {
            qDebug() << "L-return----L_Cylinder_Uping  ||  L_Horizontal_Reset" \
        ...
        if (IN_L_Start & IO_in_H) { qDebug() << "L-Start";
            if (System_ing) {
        ...
        qDebug() << QString::number(Record_Test);
        break;
    case IN_L_Stop:
        if ((IN_L_Stop & IO_in_H)) { // !
            ...
            qDebug() << "Out_Singal_H" << Out_Singal_H;
...
void pic::Get_L_In(int in_l) {
    ...
    case IN_Motor_Ready:
        if (IN_Motor_Ready & IO_in_L) { // 电机样品到位
            L_Test_Product = true; qDebug() << "L_IN_Motor_Ready";
        } else {
    ...
    case IN_Vertical_GO:
        if (Switchover_Factory == BDVacuum) {
            if (IN_Vertical_GO & IO_in_L) { // 步进电机到位
                R_Test_Product = true;  qDebug() << "R_IN_Motor_Ready";
            } else {
...
void pic::L_Exhaust_Air() {
    qDebug() << "2018-6-29 L_Exhaust_Air()";
    L_Air_Press_Hold = false;
