...
Widget_ACW::Widget_ACW(QWidget *parent) :
    ...
    ui->acwTab->setColumnWidth(3, 70);
    ui->acwTab->setColumnWidth(4, 72);
...
void Widget_ACW::Pri_ACW_Init(QString str)
{
    ...
    qDebug() << "ACW_Between_IR" << ACW_Between_IR;
    if (ACW_Between_IR) {
...
/******************************************************************************
  ...
  * brief:      OK按键槽函数
******************************************************************************/
void Widget_ACW::join_buttonJudge(int)
{
    int i;
    QStringList check;
    QString sendcheck;
...
void Widget_ACW::Pri_ACW_Save_Data()
{
    ...
    if (ACW_Between_IR) {
        for (i = 0; i < 4; i++) {
            qDebug() << "Box_ACW_list.size()" << Box_ACW_list.size();
...
void Widget_ACW::on_acwTab_cellChanged(int row,  int column)
{
    ...
    if (column  ==  1 || column  ==  2) {
        ...
        if (column == 4 || column == 5) {
            if (ui->acwTab->item(row, column)->text().toDouble() > 20) {
                ui->acwTab->item(row, column)->setText("20.00");
            }
            ui->acwTab->item(row,  column)->setText\
...
/******************************************************************************
  ...
  * brief:      测试交耐新协议
******************************************************************************/
void Widget_ACW::Pri_ACW_Test()
{
    ACW_Test_Total = 0;
    ACWgroup_num.clear();
    double Down_Limit = 0.0, Up_Limit = 0.0;
    int i, j;
    QStringList T1_H;  // -高端
    QStringList T2_H;  // -低端
    int T1_H_Data = 0;
    ...
    if (ACW_Between_IR) {
        for (i = 0; i < 4; i++) {
            ...
            frame.data[4] = int(Copy_ACW_List.at(36+i*16).toDouble()*10)/256;  // -测试时间
            frame.data[5] = int(Copy_ACW_List.at(36+i*16).toDouble()*10)%256;
            frame.data[6] = (int)((Copy_ACW_List.at(33+i*16).toDouble() + \
                                   Copy_ACW_List.at(37+i*16).toDouble())*100)/256;
            //  下限耐压单位0.01mA，绝缘单位0.1MΩ //  Copy_ACW_List.at(3)/256
            frame.data[7] = (int)((Copy_ACW_List.at(33+i*16).toDouble() + \
                                   Copy_ACW_List.at(37+i*16).toDouble())*100)%256;
            ...
            frame.data[1] = i;  // -测试组号
            frame.data[2] = (int)((Copy_ACW_List.at(34+i*16).toDouble() + \
                                   Copy_ACW_List.at(37+i*16).toDouble())*100)/256;
            //  上限耐压单位0.01mA，绝缘单位0.1MΩ  //  Copy_ACW_List.at(4)/256
            frame.data[3] = (int)((Copy_ACW_List.at(34+i*16).toDouble() + \
                                   Copy_ACW_List.at(37+i*16).toDouble())*100)%256;
        ...
        frame.data[4] = int(Copy_ACW_List.at(21).toDouble()*10)/256;  // -测试时间
        frame.data[5] = int(Copy_ACW_List.at(21).toDouble()*10)%256;
        Down_Limit = (Copy_ACW_List.at(23).toDouble() + Copy_ACW_List.at(3).toDouble());
        frame.data[6] = (int)(Down_Limit*100)/256;  // -下限耐压单位0.01mA
        frame.data[7] = (int)(Down_Limit*100)%256;
        qDebug() << "Down" << frame.data[6] << frame.data[7];
        canSend(frame);
        qDebug() << "Down_Limit" << Down_Limit;
        ...
        frame.data[2] = (int)(Up_Limit*100)/256;  // -上限耐压单位0.01mA
        frame.data[3] = (int)(Up_Limit*100)%256;
        frame.data[4] = Copy_ACW_List[0].toInt();  // -电弧等级
        frame.data[5] = 0x03;  // -缓升
        frame.data[6] = 0x0a;  // -缓降
        qDebug() << "Up" << frame.data[2] << frame.data[3];
        canSend(frame);
        qDebug() << "Up_Limit" << Up_Limit;
    }
}
...
QStringList Widget_ACW::ACW_Test_Result(QStringList ACW_Data)
{
    ...
    if (ACW_Compensate_Flag) { // -交耐补偿
        ...
        qDebug() << "acw compensate" << ACW_Show_Data-(ACW_Compensate_Result);
    } else {
        Back_QStringList.append(QString::number(ACW_Show_Data));
        qDebug() << "acw not compensate" << ACW_Show_Data;
    }
    Back_QStringList.append(QString::number(ACW_Judge));

    return Back_QStringList;
}
QStringList Widget_ACW::ACW_Test_Param()
{
    QString str;
    QStringList strSql;
