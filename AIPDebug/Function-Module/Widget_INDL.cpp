...
void Widget_INDL::Pri_INDL_Test_Parameter()
{
    ...
    for (i = 0; i < 8; i++) {
        ...
        qDebug() << "frame.data[5]------------------" << frame.data[5];
        ...
        if (ui->indlTab->item(i, 10)->text() == "mH") {
            frame.data[6] = (int)(connetct|0x09);
        } else {
            if (ui->indlTab->item(i, 4)->text().toDouble() < 500) {
                frame.data[6] = (int)(connetct|0x0f);
            } else {
                frame.data[6] = (int)(connetct|0x09);
            }
        }

        qDebug() << "frame.data[6]" << frame.data[6];
        if (i >= 8) {
            INDL_TestItemsH = INDL_TestItemsH + (1 << (i-8));   //   高字节
        } else {
            INDL_TestItemsL = INDL_TestItemsL + (1 << i);       //   低字节
...
/******************************************************************************
  ...
  * brief:      INDL界面, 测试启动
******************************************************************************/
void Widget_INDL::Pri_INDL_Test_Start()
{
...
QStringList Widget_INDL::INDL_Test_Param()
{
    ...
    if ((Copy_INDL_List[1] != "0.0") && (INDL_TestNumber >= 3)) {
        strTest.append(QString(tr("电感")));
        strParam.append(QString(tr("不平衡度 ")) + Copy_INDL_List[1] + "%");
        strSql.append(tr("电感"));
    }   // -将(-电感-)修改为(电感)
    if ((Copy_INDL_List[7].toDouble() != 0.0)&&(INDL_TestNumber >= 3)) {
