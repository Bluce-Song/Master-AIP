...
void Widget_IMP::Pri_IMP_Test()
{
    ...
    for (i = 0; i < 8; i++) {
        if (Copy_IMP_List.at(NUM_ITT*i+8).toInt() == 2) { //  判断匝间数据库是否被勾选
            ...
            if(ui->checkBox_compare->checkState()==2)
            {
            ...
            if (Copy_IMP_List[i*NUM_ITT+3].toInt() == 1) {
                frame.data[6] = ((frame.data[6] << 4) + Copy_IMP_List[i*NUM_ITT + 3].toInt()+1);
            } else {
                frame.data[6] = ((frame.data[6] << 4) + Copy_IMP_List[i*NUM_ITT + 3].toInt());
            }
            if(ui->checkBox_compare->checkState()==2) {
                frame.data[7] = Copy_IMP_List[9].toInt(); //频率 frame.data[7] = setting[OTHER+i*ITT_NUM+9].toInt();
            } else {
                frame.data[7] = Copy_IMP_List[i*NUM_ITT+9].toInt(); //频率 frame.data[7] = setting[OTHER+i*ITT_NUM+9].toInt();
            }
            qDebug() << "---Test down fre---" << (Copy_IMP_List[i*NUM_ITT + 9].toInt());
            canSend(frame);
...
void Widget_IMP::Pri_IMP_TestSample(int model, int station)
{
    ...
    qDebug() << "Pri_IMP_TestSample Finsh";
}
/******************************************************************************
  * version:    1.0
  * author:     link
...
void Widget_IMP::Pri_IMP_Init(QString File)
{
    ...
    ui->ittTab->setColumnWidth(1, 40); // 低端
    ui->ittTab->setColumnWidth(2, 40); // 高端
    ...
    ui->ittTab->setColumnWidth(7, 44); // 电晕
    ...
    if (Copy_IMP_List.at(16).toInt() == 2) {
        ui->checkBox_compare->setChecked(true);
    } else {
        //
    }
    if (Copy_IMP_List.at(17).toInt() == 2) {
        ui->checkBox_compare_2->setChecked(true);
    } else {
        //
    }
}
...
{
    int i, j;
    ui->dockWidget->hide();  IMP_Labelhide->hide();
    //更新匝间数据
    ...
    if (ui->checkBox_compare_2->checkState() == 2) {
...
void Widget_IMP::Pri_IMP_SetSample_Sarameter(int i, int volt)
{
    ...
    frame.data[5] = (volt)%256; qDebug() << "---down volt---" << volt;
...
void Widget_IMP::Pri_IMP_Init_Join()
{
...
void Widget_IMP::Pri_IMP_Default_value(QString number) // 匝间设置默认数值
{
    int i;
    for (i = 0; i < 8; i++) {
        ui->ittTab->item(i, 3)->setText("1800");  //  设置默认电压 1800
    ...
    for (i = 0; i < number.size()/2; i++) {
        ui->ittTab->item(i, 1)->setText(number.mid(i*2, 1));
        ui->ittTab->item(i, 2)->setText(number.mid(i*2+1, 1));
        Box_IMP_List[i]->setChecked(true);
    }
}
/******************************************************************************
...
void Widget_IMP::on_button_sample_average_clicked()
{
    ...
    qDebug() << "sample_time" << sample_time;
    if ((sample_time-1) >= 1) {
...
QStringList Widget_IMP::Pub_Main_IMP_Result(int ittwaveframe, int *waveItt, int station)
{
    ...
    if (station == 0x13) {
        ...
        imp_result.replace(3, QString::number\
                           ((double)(Pri_IMP_Phase(IMP_WaveSqlList[ittwaveframe]->point, \
                                          waveItt, IMP_origin, IMP_terminal)*100/(100+Area)), 'f', 0));
        ...
        imp_result.replace(2, QString::number(Pri_IMP_Flut(waveItt, IMP_origin, IMP_terminal)));
        imp_result.replace(3, QString::number\
                           ((double)(Pri_IMP_Phase(IMP_Right_Wave[11]->sample_point[ittwaveframe], \
                            waveItt, IMP_origin, IMP_terminal)*100/(100+Area)), 'f', 0));
    } else {
        //
    }
...
QStringList Widget_IMP::Pub_Main_IMP_Result_Compare(int ittwaveframe, \
    ...
    imp_result.replace(1, QString::number(Pri_IMP_Diff(Compare_waveItt, waveItt, \
                                                       IMP_origin, IMP_terminal), 'f', 1));
    imp_result.replace(2, QString::number(Pri_IMP_Flut(waveItt, IMP_origin, IMP_terminal)));
    imp_result.replace(3, QString::number((double)(Pri_IMP_Phase(Compare_waveItt, \
                                  waveItt, IMP_origin, IMP_terminal)*100/(100+Area)), 'f', 0));
...
{
    qDebug() << "tttttttttttt" << ittwaveframe;

    QStringList imp_result;  imp_result<<"0"<<"0"<<"0"<<"0";
    double Area = 0.0;
    int IMP_origin,IMP_terminal;
    if (ittwaveframe==0) {
        qDebug() << "2222222222222222-----0";
        IMP_origin=(Copy_IMP_List.at(NUM_ITT*ittwaveframe+11).toInt())*400/651;
        IMP_terminal=(Copy_IMP_List.at(NUM_ITT*ittwaveframe+12).toInt())*400/651;
        if(station==0x13)
        {
            Area = Pri_IMP_Area(IMP_WaveSqlList[ittwaveframe]->point,waveItt,IMP_origin,IMP_terminal);
            imp_result.replace(0,QString::number(Area,'f',1));
            imp_result.replace(1,QString::number(Pri_IMP_Diff(IMP_WaveSqlList[ittwaveframe]->point,waveItt,IMP_origin,IMP_terminal),'f',1));
            imp_result.replace(2,QString::number(Pri_IMP_Flut(waveItt,IMP_origin,IMP_terminal)));
            imp_result.replace(3,QString::number((double)(Pri_IMP_Phase(IMP_WaveSqlList[ittwaveframe]->point,waveItt,IMP_origin,IMP_terminal)*100/(100+Area)), 'f', 0));
        }
        else if(station!=0x13)
        {
            Area = Pri_IMP_Area(IMP_Right_Wave[11]->sample_point[ittwaveframe],waveItt,IMP_origin,IMP_terminal);
            imp_result.replace(0,QString::number(Area,'f',1));
            imp_result.replace(1,QString::number(Pri_IMP_Diff(IMP_Right_Wave[11]->sample_point[ittwaveframe],waveItt,IMP_origin,IMP_terminal),'f',1));
            imp_result.replace(2,QString::number(Pri_IMP_Flut(waveItt,IMP_origin,IMP_terminal)));
            imp_result.replace(3,QString::number((double)(Pri_IMP_Phase(IMP_Right_Wave[11]->sample_point[ittwaveframe],waveItt,IMP_origin,IMP_terminal)*100/(100+Area)), 'f', 0));
        }
        else
        {
        ...
        Area = Pri_IMP_Area(Compare_waveItt,waveItt,IMP_origin,IMP_terminal);
        imp_result.replace(0,QString::number(Area,'f',1));
        imp_result.replace(1,QString::number(Pri_IMP_Diff(Compare_waveItt,waveItt,IMP_origin,IMP_terminal),'f',1));
        imp_result.replace(2,QString::number(Pri_IMP_Flut(waveItt,IMP_origin,IMP_terminal)));
        imp_result.replace(3,QString::number((double)(Pri_IMP_Phase(Compare_waveItt,waveItt,IMP_origin,IMP_terminal)*100/(100+Area)), 'f', 0));
...
{
    if (sample_time == 1) {
        ui->label->setText(tr("默认--采集完成")); qDebug() << "IMP save data";
