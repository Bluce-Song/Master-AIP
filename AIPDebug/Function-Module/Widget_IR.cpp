...
/******************************************************************************
  ...
  * brief:      测试绝缘新协议
******************************************************************************/
void Widget_IR::Pri_IR_Test(QString str)
{
    int i,  j;
    QStringList T1_H; // -高端
    QStringList T2_H; // -低端
    ...
    if (ACW_Between_IR) {
        for (i = 0; i < 4; i++) {
            ...
            if (Copy_IR_List.at(30+i*10) == "PE" || \
                ...
                qDebug() << Copy_IR_List.at(30 + i*10);
                ...
                qDebug() << Copy_IR_List.at(31 + i*10);
        ...
        frame.data[0] = 0x04; // -绝缘测试参数2
        frame.data[1] = 0x01; // -测试组号
        frame.data[2] = ui->ComboBox_IR_Volt->currentText().toInt()/256;
        frame.data[3] = ui->ComboBox_IR_Volt->currentText().toInt()%256;
...
void Widget_IR::Pri_IR_Init(QString str)
{
    ...
    ui->doubleSpinBox_IR_UpDCR->setMaximum(9999);   // 1.0-1000
...
QStringList Widget_IR::IR_Test_Param()
{
    QString str;
    QStringList strSql;
    ...
    if (ACW_Between_IR) {
        ...
        str.clear();
        str += ui->ComboBox_IR_Volt->currentText();   str += "V,";
        str += ui->doubleSpinBox_IR_UpDCR->text();  str += "MΩ,";
        // 2017.3.10 修改显示下限
        str += ui->doubleSpinBox_IR_Time->text(); str += "s";
        strParam.append(str);
        str.clear();
        strSql.append(tr("绝缘"));
    }
...
void Widget_IR::Pub_Conf_Set_IR(QString string, int value)
{
    switch (value) {
    case 1:
        Pri_IR_Init(string);
