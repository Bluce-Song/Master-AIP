    ...
    ui(new Ui::Widget_DLR)
{
    ...
    DCR_Widget->setStyleSheet("border-radius: 10px;padding:2px 4px;background-color: gray;color: black;border-color: black;");
...
void Widget_DLR::Pri_DLR_Init(QString dcr15)
{
    ...
    DLR_Meterial.clear();
    DLR_Meterial << QString(tr("铜")) << QString(tr("铝")) << QString(tr("铜铝")); // 电阻材料
    ...
    qDebug() << "Copy_DLR_List.at(10).toInt()" << Copy_DLR_List.at(10).toInt();
...
void Widget_DLR::Pri_DLR_Save_Data()
{
    ...
    Copy_DLR_List.replace(10, QString::number(DCR_DoubleSpinBox[0]->value()));   //  电阻不平衡度设置参数
...
{
    double Temp = 0.0;
    qDebug() << "RealTime_Compensate--------------Temp" << Get_Temp - 50.0;
    ...
    qDebug() << "Back--------------Temp" << Temp;
    return Temp;
}
QStringList Widget_DLR::DCR_Test_Result(can_frame canframe,  int Back_out_station)
{
    ...
    if ((canframe.data[2] == 1) || (canframe.data[2] == 2)) { // -档位（1，2）-"mΩ"
        ...
        qDebug() << "canframe.data[2]" << canframe.data[2];
    }
 /*   if (canframe.data[1] == 0) {
    ...
    if (DLR_Temp_Flag) { // 温度补偿
        ...
        dlr_data = dlr_data*(RealTime_Compensate((((double)(canframe.data[5]*256 + \
                                                   canframe.data[6]))/10), canframe.data[1]));
    }

    if ((Back_out_station == 0x13)) {  // 线路补偿  (DLR_Compensate_Left) && // 线路补偿在温度补偿之后
...
QStringList Widget_DLR::DCR_Test_Param()
{
    ...
    strDLR_UpDown.append(Copy_DLR_List.at(7));
    if (Copy_DLR_List.at(10).toInt() == 1) {
        if ((Copy_DLR_List.at(7) != "0.000") && (DLR_TestNumber >= 3)) {
            strTest.append(QString(tr("电阻")));
            strParam.append(QString(tr("不平衡度 ")) + Copy_DLR_List.at(7) + "%");
            strSql.append(tr("电阻"));
        }   //-将(-电阻-)修改为(电阻)
    } else if (Copy_DLR_List.at(10).toInt() == 2) {
        if ((Copy_DLR_List.at(7) != "0.000") && (DLR_TestNumber >= 2)) {
            strTest.append(QString(tr("电阻")));
            strParam.append(QString(tr("不平衡度 ")) + Copy_DLR_List.at(7) + "%");
            strSql.append(tr("电阻"));
        }   //-将(-电阻-)修改为(电阻)
