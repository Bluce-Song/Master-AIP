...
void Widget_BLOCK::Volt_sample() {
    ...
    frame.data[3] = ((4<<4)|((int)(ui->BLOCK_Volt->value())/256)); // 电压
...
void  Widget_BLOCK::Pri_Test_Two() {
    ...
    frame.data[1] = ui->DZ_grade->value();   // 档位
    frame.data[2] = 1000/10; // 时间
    frame.data[3] = ((4<<4)|((int)(ui->BLOCK_Volt->value()+Volt_Line_Text[0]->value())/256)); // 电压
    frame.data[4] = (int)(ui->BLOCK_Volt->value()+Volt_Line_Text[0]->value())%256;
    canSend(frame);
}
...
void Widget_BLOCK::Pub_Conf_Set_BLOCK(QString str, int value, int flag)
{
    ...
    case 8:
        if (Volt_Sample_ONE) {
            Volt_Sample_ONE = false;
            Volt_Line_Text[0]->setValue((ui->BLOCK_Volt->value())-Volt_Back_Lable->text().toInt());
            Volt_ONE = (ui->BLOCK_Volt->value())-Volt_Back_Lable->text().toInt();
            Pri_Test_Two();
        }
        else {
            Volt_Line_Text[0]->setValue((ui->BLOCK_Volt->value())-Volt_Back_Lable->text().toInt() + Volt_ONE);
