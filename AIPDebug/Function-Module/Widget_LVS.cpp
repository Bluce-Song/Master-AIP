    ...
    ui(new Ui::Widget_LVS)
{
    ...
    IO_button_sample->setText(tr("采集"));      IO_button_sample->setMinimumHeight(50);     IO_button_sample->setMinimumWidth(90);       IO_button_sample->setStyleSheet("background: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1,stop: 0 #5B5F5F, stop: 0.5 #0C2436,stop: 1 #27405A);");
...
void Widget_LVS::Volt_sample() {
    ...
    frame.data[2] = ((int)(ui->lvsTab->item(0,4)->text().toDouble()*10))/256;
    frame.data[3] = ((int)(ui->lvsTab->item(0,4)->text().toDouble()*10))%256;
    /*电源设置电压*/// 内部
    frame.data[4] = ((4<<4)|((int)(ui->LVS_Volt->value())/256));
    frame.data[5] = ((int)(ui->LVS_Volt->value()))%256;
    /*设置转向*/
    frame.data[6] = 0;
    frame.data[7] = 0;
    canSend(frame);
...
void  Widget_LVS::Pri_Test_Two() {
    ...
    frame.data[3] = ((int)(ui->lvsTab->item(0,4)->text().toDouble()*10))%256;
    /*电源设置电压*/// 内部
    frame.data[4] = ((4<<4)|((int)(ui->LVS_Volt->value()+Volt_Line_Text[0]->value())/256));
    frame.data[5] = ((int)(ui->LVS_Volt->value()+Volt_Line_Text[0]->value()))%256;
    /*设置转向*/
    frame.data[6] = 0;
    frame.data[7] = 0;
    canSend(frame);
...
void Widget_LVS::Pub_Conf_Set_LVS(QString str, int flag)
{
    ...
    case 8:
        if (Volt_Sample_ONE) {
            Volt_Sample_ONE = false;
            Volt_Line_Text[0]->setValue((ui->LVS_Volt->value())-Volt_Back_Lable->text().toInt());
            Volt_ONE = (ui->LVS_Volt->value())-Volt_Back_Lable->text().toInt();
            Pri_Test_Two();
        }
        else {
            Volt_Line_Text[0]->setValue((ui->LVS_Volt->value())-Volt_Back_Lable->text().toInt() + Volt_ONE);
