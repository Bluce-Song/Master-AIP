    ...
    ui(new Ui::Widget_PWR)
{
    ...
    Volt_Widget->setStyleSheet("border-radius: 10px;padding:2px 4px;background-color: gray;color: black;border-color: black;");
    QGridLayout *IO_upside = new QGridLayout;
    QString IO_table[6]={tr("补偿数值(V):"),tr("补偿数值(V):"),tr("补偿数值(V):"),tr("补偿数值(V):"),tr("补偿数值(V):"),tr("补偿数值(V):")};
    ...
    for(i=0;i<6;i++)
    {
        ...
        Volt_Button[i]->setStyleSheet("background: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1,stop: 0 #5B5F5F, stop: 0.5 #0C2436,stop: 1 #27405A);");
        ...
        if(i<6)
        {
            ...
            Volt_Line_Text[i]->setMaximumHeight(35);      Volt_Line_Text[i]->setMaximumWidth(100); // IO_Line_Text[i]->setMaximumWidth(51);
    ...
    IO_button_quit->setText(tr("退出"));      IO_button_quit->setMinimumHeight(50);     IO_button_quit->setMinimumWidth(90);       IO_button_quit->setStyleSheet("background: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1,stop: 0 #5B5F5F, stop: 0.5 #0C2436,stop: 1 #27405A);");
...
void Widget_PWR::Volt_sample(int i) {
    ...
    /*设置档位*/
    frame.data[1] = i; // 左工位
    frame.data[2] = ((int)(ui->pwrTab->item(i-1,8)->text().toDouble()*10))/256; //
    frame.data[3] = ((int)(ui->pwrTab->item(i-1,8)->text().toDouble()*10))%256;
    /*电源设置电压*/// 内部
    frame.data[4] = ((4<<4)|((int)(ui->PWR_Volt->value())/256));
    frame.data[5] = ((int)(ui->PWR_Volt->value()))%256;
    /*设置转向*/
    frame.data[6] = 0;
    frame.data[7] = 0;
    canSend(frame,0);
...
void  Widget_PWR::Pri_Test_Two() {
    ...
    frame.data[2] = ((int)(ui->pwrTab->item(Volt_Sample_Grade-1,8)->text().toDouble()*10))/256; //
    frame.data[3] = ((int)(ui->pwrTab->item(Volt_Sample_Grade-1,8)->text().toDouble()*10))%256;
    /*电源设置电压*/// 内部
    frame.data[4] = ((4<<4)|((int)(ui->PWR_Volt->value()+Volt_Line_Text[Volt_Sample_Grade-1]->value())/256));
    frame.data[5] = ((int)(ui->PWR_Volt->value()+Volt_Line_Text[Volt_Sample_Grade-1]->value()))%256;
    /*设置转向*/
    frame.data[6] = 0;
    frame.data[7] = 0;
    canSend(frame,0);
...
// PG COMMON   // PG电机 普通电机
{
    ...
    ui->noise_time->setValue(Copy_PWR_List.at(31).toDouble());

    if(Copy_PWR_List.at(32).toInt()==2)
    {
        ui->noise_enable->setChecked(true);
    }
    else
    {
        ui->noise_enable->setChecked(false);
    }
    for (i = 0; i < PWM_Row_Count; i++) {
...
void Widget_PWR::Pri_PWR_Data_Save()
{
    ...
    Copy_PWR_List.replace(31,QString::number(ui->noise_time->value())); // noise--
    Copy_PWR_List.replace(32,QString::number(ui->noise_enable->checkState())); // noise--
    for (i = 0; i < PWM_Row_Count; i++) {
...
void Widget_PWR::Pub_Conf_Set_PWR(QString str, int flag)
{
    ...
    case 8:
        if (Volt_Sample_ONE) {
            Volt_Sample_ONE = false;
            Volt_Line_Text[Volt_Sample_Grade-1]->setValue((ui->PWR_Volt->value())-Volt_Back_Lable->text().toInt());
            Volt_ONE = (ui->PWR_Volt->value())-Volt_Back_Lable->text().toInt();
            Pri_Test_Two();
        }
        else {
            Volt_Line_Text[Volt_Sample_Grade-1]->setValue((ui->PWR_Volt->value())-Volt_Back_Lable->text().toInt() + Volt_ONE);
