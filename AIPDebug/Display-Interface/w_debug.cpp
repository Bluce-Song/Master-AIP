...
void w_Debug::R_Debug_DLRData(int data)
{
    if (ui->tabWidget2->currentIndex() == 1) {  //  电阻测试结果返回
        if (ZeroAndDLR == 1) {
            ui->DLRBox_0->setValue((double)(data));
        }
        if (ZeroAndDLR == 2) {
            ui->DLRBox->setValue((double)(data));
...
void w_Debug::Debug_TestINS_CAN(int Itemnumb, int Type, int Data, int Grade)
{
    ...
    qDebug() << "qDebug" << Freq << DownLimit << Toplimit;
    frame.can_id  = 0x23;
    frame.can_dlc = 0x08;
    ...
    canSend(frame, wDebug_Surface);
    qDebug() << frame.data[2] << frame.data[3];
    ...
    qDebug() << "qDebug" << Type << Grade << Itemnumb;
    canSend(frame, wDebug_Surface);
}
...
void w_Debug::Debug_IMP_KB(int id, int Grade, int K, int B)
{
    ...
    qDebug() << frame.data[1] << frame.data[2] << frame.data[3] << frame.data[4] << frame.data[5];
    canSend(frame, wDebug_Surface);
}
/******************************************************************************
  * version:    1.0
...
{
    qDebug() << "Debug_TestIMP_CAN" << data << Grade;
...
void w_Debug::on_checkBox_11_clicked()
{
    qDebug() << "Erroe";
}
