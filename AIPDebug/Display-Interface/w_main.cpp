...
void w_Main::Pubs_from_conf(QStringList list, QString str, int data, int value) {
    ...
    case 2:
        Pubs_ChangeWindow(data);
        Signal_main_to_Switchover(KaiYu, 0x00, 6);
        break;
    case 3:
...
void w_Main::Pri_Synchron_Result_Data() {
    ...
    for (i = 0; i < Ini_Proj.size(); i++) {
        ...
        case 7: // 电参
            ...
            for(int j=0;j<wTest->PG_Test_Order.size();j++)
            {
...
void w_Main::Pubs_ChangeWindow(int win) {
    qDebug() << "Pubs_ChangeWindow win-->" << win;
...
//        canSend(frame, 88);
    ...
    case 8: //-设置界面
        ...
        qDebug() << "wConf";
        Signal_main_to_Switchover(KaiYu, 0x03, 6);
...
void w_Main::ReadMessage(quint16 addr, quint16 cmd, QByteArray msg)
{
    ...
    case CMD_All_Stop:
        ...
        break;
    case CMD_WMessage:
        qDebug() << "WMessage_Record" << WMessage_Record;
    ...
    case CMD_MAG_Sample:
        ...
        break;
    case CMD_IMP_Sample: // 匝间采样
        Back_out_station = n.at(1).toInt();
        Signal_main_to_Switchover(BDVacuum, n.at(1).toInt() - 18, 9);
    ...
    case CMD_IMP_Finsh: // 匝间采集完成
        ...
        Signal_main_to_Switchover(BDVacuum, 0, 9);
    ...
    case CMD_A3:
        ...
        break;
    case CMD_A4:
        WriteMessage((6000), 0xa4, QString::number(wpic->Record_Test).toUtf8());
...
//                Qt::QueuedConnection);
    ...
    default:
        ...
        if (sql_error_count%10 == 0) {
            sql_error_count = 0;
            if (wConf->SQL_Error) {
                wTest->Pub_Main_to_test(QStringList(""),wConf->SQL_Error_Test,21,21);
            }else {
                //
            }
//            else if(MYSQL_DB->SQL_Error) {
//                           wTest->Pub_Main_to_test(QStringList(""),MYSQL_DB->SQL_Error_Test,21,21);
//                       }
        } else {
            //
        }
...
void w_Main::Pubs_from_pic(QString string, int value, int flag) {
    ...
    case 9:
        ...
        break;
    case 10:
        WriteMessage((6000), CMD_WMessage, QString::number(value).toUtf8());
        break;
    default:
...
void w_Main::Pri_Board_Out(int count, int Station)
{
    if (can.frame.data[0] == CAN_UPLOAD_STATE) {          //-上传状态
        ...
        if (can.frame.data[1] == Receive_Stop) {
            if (window_current == wTest_Surface) {        //-测试界面
                ...
                Signal_main_to_Switchover(LvZhi, 0x12, 2);
                Signal_main_to_Switchover(WeiLing, out_station, 3);
            ...
            if ((window_current == wTest_Surface) && (!isTesting)) { // -测试界面,已经拉开滑罩
                ...
                Signal_main_to_Switchover(LvZhi,out_station,2);
                Signal_main_to_Switchover(BePuDa,out_station,12);
            } else {
                //
            }
        } else {
   ...
   //     qDebug()<<"Reset--0x02 "<< system_conf_list.at(4).toInt();
        WriteMessage((6000), CMD_WMessage, QString("16").toUtf8());
...
void w_Main::Pri_Board_IMP()
{
    ...
    if (can.frame.data[0] == CAN_UPLOAD_STATE) {
     ...
     //   wTest->State.IMP = true;
        ...
        if (window_current != wDebug_Surface) {
            if (can.frame.data[1] == 255) {
                if ((imp_back_number == 0) && (wConf->WIMP->Copy_IMP_List.at(16).toInt() == 2)) { //- 比较模式
                    for (int i = 0; i < 400; i++) {
...
void w_Main::PG_State_Check() {
    qDebug() << "2018-4-11 PG_State_Check --- Current State" << wTest->PG_Test_State;
    ...
    if (can.frame.data[0] == CAN_UPLOAD_STATE) {
        ...
        if (window_current == wTest_Surface) {
            if (can.frame.data[1] == 0xA0) { // 测试状态
                ...
                qDebug() << "2018-4-11 WD_Read_Param";
                ...
                qDebug() << "PG_State_Error" << can.frame.data[1];
...
void  w_Main::Pri_Board_IO() {
    ...
    //    wTest->Pub_Main_to_test(QStringList(""), QString(""), in_data, 120);
        ...
        WriteMessage((6000), CMD_WMessage, QString("17").toUtf8());
    }  else {
        //
    }
}
...
void  w_Main::Pri_Main_WMessage(int number, QString str) {
    qDebug() << "wTest->START_Model" << wTest->START_Model;
