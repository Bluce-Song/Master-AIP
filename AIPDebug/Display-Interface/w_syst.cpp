    ...
    ui(new Ui::w_Syst)
{
    ...
    QString net_table[4] = {"ip:", QString(tr("网关:")), "DNS:" ,QString(tr("自动获取"))};
    int i = 0;
    for (i = 0; i < 4; i++) {
    ...
    QString IO_table[6]={QString(tr("延时时间(Ms):")), \
                         QString(tr("电阻平衡度:")), \
                         QString(tr("不合格弹起")), QString(tr("防呆:")), \
                        QString(tr("电阻1.5")), QString(tr("恢復出厂")) };
    for (i = 0; i < 6; i++) {
    ...
    NG_Push->setStyleSheet("QCheckBox::indicator {image: url(:/image/053.png);width: 55px;height: 45px;}QCheckBox::indicator:checked { image: url(:/image/051.png); }QCheckBox  { color: rgb(0, 0, 0);}");
    ...
    dcr_Disable->setStyleSheet("QCheckBox::indicator {image: url(:/image/053.png);width: 55px;height: 45px;}QCheckBox::indicator:checked { image: url(:/image/051.png); }QCheckBox  { color: rgb(0, 0, 0);}");
    ...
    button_recover->setStyleSheet\
            ("background: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1,"\
             "stop: 0 #5B5F5F, stop: 0.5 #0C2436,stop: 1 #27405A);");
    IO_upside->addWidget(button_recover, 5, 1);
...
void w_Syst::net_Widget_set_net()
{
    ...
    QString DHCP;
    QString line_data;
    QString IPAddress; IPAddress = Set_qq + line_Text.at(0)->text() + Set_qq;
    QString DefaultGateway; DefaultGateway = Set_qq + line_Text.at(1)->text() + Set_qq;
    QString SubnetMask; SubnetMask = Set_qq + line_Text.at(2)->text() + Set_qq;
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        while (!in.atEnd()) {
            ...
            if (line.contains("DefaultGateway=", Qt::CaseInsensitive)) {
                line.replace(QRegExp("DefaultGateway=.*"),QString("DefaultGateway=") + DefaultGateway);
            }
...
void w_Syst::on_Button_Check_3_clicked()
{
    ...
    Get_Static_DNS = set_userinfo->value("LOCAL_MACHINE/SubnetMask",QString("255.255.255.0")).toString();
    delete set_userinfo;
    ...
    line_Text.at(1)->setText(Get_DefaultGateway); //  网关
    line_Text.at(2)->setText(Get_Static_DNS); //  DNS
    net_Widget->show();
}
