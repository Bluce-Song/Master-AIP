...
void w_Test::InitTest_Interface(QStringList Data_List)
{
    ...
    if (Conf_Item.contains("7")) {
        if (strPWR.size() != 0) {
           Noise_Time = strPWR.at(31).toDouble(); // noise--
...
void w_Test::Test_Process() {
 ...
 // Modification_Soft_Askoll(Station);
    ...
    if ((Ini_IO_Model==Noise)&&(Test_noise_enabled)&&(Noise_Time!=0)) { //
        qDebug() << "noise";
        Signal_test_to_Switchover(Noise,Station_Chose - 0x13 + 1,7);
        Noise_Stop->setSingleShot(TRUE);
        Noise_Stop->start(Noise_Time*1000); //
        ui->label_noise->show();
    }
    else {
...
//    bool All_State = (State.DLR && State.IMP && State.IR);
    if (Test.Status_Ing == Test_Station_Ask) {
        Test.Status_Ing = Test_Station_Busy;
        Ask_Widget->hide();
        label_Waring->hide();
        ...
        qDebug() << "3";
//        if (!(All_State)) {
//            qDebug() << "退出" << "State.DLR" << State.DLR << \
//                        "State.IMP" << State.IMP << "State.IR" << State.IR;
//         // Test_WMessage(11, QString(tr(""))); // 10
//            WriteMessage((6000), 0x23, QString::number(2100*11).toUtf8());
//            Ask_Up_label->setAlignment(Qt::AlignCenter);
//            Ask_Up_label->setText(tr("功能板异常\n停止启动"));
//            Ask_Widget->show();
//            return;
//        }
        ...
        if (Ini_IO_Test) { // 启动信号启动
            ...
            qDebug()<<"canSend(frame,wTest_Surface);";
...
/******************************************************************************
  ...
  * brief:      匝间波形参数
******************************************************************************/
void w_Test::Pub_IMP_Param(QStringList imp_data, int *waveform_point)
{
    int i = 0;
    ...
    if (strITT.at(16).toInt() == 2) {
        ...
        if (imp_back_number == 0) {
            ...
            if (Station_Chose == 0x13) {
                for (i = 0; i < 400; i++) {
                    IMP_Left_List[0]->sample_point[Imp_Compare_Model][i] = waveform_point[i]/4;
                }
            } else {

                for (i = 0; i < 400; i++) {
                    IMP_Right_List[0]->sample_point[Imp_Compare_Model][i] = waveform_point[i]/4;
                }
            }
        }
    } else {
